...
public class RentDetailFavoriteFragment extends Fragment
{
	int mNum;
	String mRentTypeKey;
	...
	private OrmRentHouse theOrmRentHouse;

	/**
	 * Create a new instance of CountingFragment, providing "num" as an
	 * argument.
	 */
	public static RentDetailFavoriteFragment newInstance(int num,
			FavoriteDetailActivity theActivity)
	{
		RentDetailFavoriteFragment f = new RentDetailFavoriteFragment();
	...
	public void onCreate(Bundle savedInstanceState)
	{
		...
		try
		{
			Dao<OrmRentHouse, Integer> rentDao = mActivity.getHelper()
					.getOrmRentHouseDao();
			ArrayList<OrmRentHouse> lists = new ArrayList<OrmRentHouse>(
					rentDao.queryForAll());
			theOrmRentHouse = lists.get(mNum);
			...
			Bundle savedInstanceState)
	{
		...
		// set image
		String x_long = Double.toString(theOrmRentHouse.x_long);
		String y_lat = Double.toString(theOrmRentHouse.y_lat);
		...
		if (getIsAddFavorite(theOrmRentHouse.rent_id))
		{
	...
	private class GetEstatesTask extends AsyncTask<Void, Void, Void>
	{
		...
		protected Void doInBackground(Void... arg)
		{
			...
			RentHouse newRentHouse = HouseApi
					.getHouseDetails(theOrmRentHouse.rent_id);
		...
		protected void onPostExecute(Void arg)
		{
			...
			} else
			{
				...
				String moneyString = "<font size=\"3\" color=\"red\">"
						+ Integer.toString(theRentHouse.price) + "元/月"
						+ "</font>" + ",&nbsp;"
				...
				String layerString = InfoParserApi.parseLayers(
						theRentHouse.layer, theRentHouse.total_layer);
				...
				featureTextView.setText(Html
						.fromHtml(theRentHouse.feature_html));
				...
				{
					otherString = otherString + "*身份限制:"
							+ theRentHouse.identity + "\n";
				}
				...
				if (!theRentHouse.orientation.equals("")
						&& !theRentHouse.sexual_restriction.equals("null"))
				{
					otherString = otherString + "*房子座向:"
							+ theRentHouse.orientation + "\n";
				}
				...
				{
					furnitureTextView
							.setText("提供傢俱: " + theRentHouse.furniture);
				} else
				{
	...
	private void showDataMissDialog()
	{
		...
		builder.setPositiveButton("刪除", new DialogInterface.OnClickListener()
		{
			...
			{
				Dao<OrmRentHouse, Integer> rentHouseDao;
				try
				{
					rentHouseDao = mActivity.getHelper().getOrmRentHouseDao();
					DeleteBuilder<OrmRentHouse, Integer> deleteBuilder = rentHouseDao
							.deleteBuilder();
					deleteBuilder.where().eq(OrmRentHouse.Column_Rent_ID_NAME,
							theRentHouse.rent_id);
					deleteBuilder.delete();
