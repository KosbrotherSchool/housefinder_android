...
public class ListRentHouseAdapter extends BaseAdapter
{

	private final Activity activity;
	private final ArrayList<RentHouse> data;
	private static LayoutInflater inflater = null;
	private ImageLoader imageLoader;

	// private String mChannelTitle;

	public ListRentHouseAdapter(Activity a, ArrayList<RentHouse> d)
	{
		activity = a;
		data = d;
	...
	public View getView(final int position, View convertView, ViewGroup parent)
	{
		...
		String moneyString = "<font size=\"3\" color=\"red\">"
				...
				+ InfoParserApi.parseRentArea(data.get(position).rent_area)
				+ "坪" + "</font>";
		textMoney.setText(Html.fromHtml(moneyString));
		
		String typeString = "<font size=\"3\" color=\"black\">"
				+ InfoParserApi.parseRentType(data.get(position).rent_type_id)
				;
		
		if (InfoParserApi.parseRoomArrangement(data.get(position).rooms,
				0, data.get(position).rest_rooms, 0)!="")
		{
			typeString  = typeString + ",&nbsp;" + InfoParserApi.parseRoomArrangement(data.get(position).rooms,
					0, data.get(position).rest_rooms, 0);
		}
		...
		{
			typeString  = typeString + ",&nbsp;" + InfoParserApi.parseLayers(data.get(position).layer,
					data.get(position).total_layer);
		}
